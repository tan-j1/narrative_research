#增加了匹配模式Who are the sharks（5、7季节适用，8、9仍然不适用）！！！！！！！
import os
import re
...
def remove_content_before_first(source_folder, target_folder):
    ...
    for filename in os.listdir(source_folder):
        if filename.endswith('.txt'):
            ...
            # 使用宽松的正则表达式查找两种可能的模式
            pattern = r'first[\s,.]*in[\s,.]*the[\s,.]*tank|first[\s,.]*into[\s,.]*the[\s,.]*tank|who[\s,.]*are[\s,.]*the[\s,.]*sharks'  
...
# 使用示例
source_folder = r'e:\中科院\shark tank视频下载\raw_data season5'
target_folder = r'e:\中科院\shark tank视频下载\raw_data season5_1'
